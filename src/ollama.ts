...
function buildSectionE(playerCommand: string, locationExits: { label: string; target: string; direction?: string }[]): string {
  const exitLine =
    locationExits.length > 0
      ? `\nMOVEMENT: If the player's command includes going through a door/exit or a direction (e.g. "go through door", "east", or "take X and go through door"), set the player's new_location_id to the exact exit target from EXITS (e.g. "kitchen"). Apply every part of a compound command—if they take something and go through the door, you must set both the object's new_location_id to "player_inventory" and the player's new_location_id to the exit target. Without the player's new_location_id the engine will not move them.\n`
      : "";
