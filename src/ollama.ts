...
- In narrative_prose, describe EVERY exit in EXITS FROM THIS LOCATION (direction and destination) so the player can move. The list includes every exit from this room, including the way the player entered (e.g. south back to scriptorium). You MUST mention each one—never say "no exits" or "no obvious exits" if the list has exits. Mention only the listed exits—no extra doors or directions.
...
`;
  ...
  if (exits.length === 0) {
    ...
    out += `\nEXITS FROM THIS LOCATION (only these exist; do not invent others). The list includes every exit from this room, including the way the player entered. Your narrative_prose MUST describe each exit below—direction and where it leads (e.g. "To the south, a heavy door leads back to the scriptorium") so the player can move:\n`;
