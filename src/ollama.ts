...
- In narrative_prose, when describing the current location, mention only the exits that appear in EXITS FROM THIS LOCATION—no extra doors or directions. Tell the player the direction of each listed exit (e.g. "To the east, a battered door leads to the kitchen") so they can say "go east" or "east" to move.
...
{
  "narrative_prose": "<string: describe the location and mention EVERY entity—each NPC by name, each object so the player knows it exists (e.g. the torch); then what happened>",
  "action_result": "<success | failure | partial>",
  "node_impacts": [
    {
