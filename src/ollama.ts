...
- Movement is NOT a take. When the player only moves (e.g. "east", "go through the door", "north", "leave", "go to kitchen"), do NOT set new_location_id to "player" for any object; do NOT write "The player now holds the X" or "Taken by player." for any object; objects stay where they are. The engine strips object moves on pure movement turns—do not invent taking or giving on those turns.
- **Object location → player (warranted transfers):** Set new_location_id to "player" on the **object** when the transfer is **warranted** by the player's input and the outcome you narrate—not only when they used a bare imperative. Warranted when: (1) they explicitly commanded taking that object (e.g. "take torch", "get the key"); (2) your narrative resolves an **NPC handing or giving** the item to the player; (3) **dialogue or action** commits to receiving or picking it up (e.g. "Give me that," "I'll fetch light," "I'll get you some light" and you resolve by taking the torch); (4) another transfer **authentic** to the scene. **Do not** set new_location_id without such a warrant—no unmotivated or narrator-convenient grabs. When taking from a container, set new_location_id to "player" only for the taken object, not the container. Do NOT put new_location_id on the player entry for a take. Do NOT add an adjective to the object for the move alone. Taking does not imply lighting/using: state changes (lit, open, etc.) still require an explicit action for that state. When you set new_location_id to "player", use exactly: narrative_prose "The player now holds the [object]."; that object's prose_impact "Taken by player."
...
- **NPC takes player's item:** Set the object's new_location_id to the **NPC** only when the player **warrants** giving it—e.g. explicit "give X to Ciaran" or dialogue/action you resolve as a real handover. If they only talk about or mention the item without giving, the NPC does not receive it.
...
function buildSectionE(
  ...
  const offerBlock = isOfferOrQuestion
    ...
    : "";
  const explicitTakeTorch =
    /\b(take|get|grab|pick\s+up)\s+(?:the\s+|an\s+|a\s+)?(?:unlit\s+|lit\s+)?torch\b/i.test(playerCommand);
  const dialogueSpeechBlock =
    !isOfferOrQuestion &&
    playerCommand.trim().startsWith('"') &&
    !explicitTakeTorch
...
Perform ONLY the action the player asked for. Resolve **object location and inventory** when **warranted** by their input and the outcome you narrate (explicit imperatives, dialogue, NPC handing over an item, committing to pick something up, etc.). Do not invent object moves with no warrant. Pure movement alone ("east") does not move objects—the engine enforces that.
${offerBlock}${dialogueSpeechBlock}${describeOnlyBlock}${inventoryOnlyBlock}START/BEGIN: If the player said "start" or "begin", describe the full scene including every NPC and object in ENTITIES PRESENT by name. Do NOT set new_location_id for any object. No state changes.
${containmentLine}OBJECT → PLAYER: Set new_location_id to "player" on the **object** when **warranted** (see ROLE rules: explicit take, NPC gives, dialogue commits to receiving/picking up, etc.)—**not** when the player only moved (direction alone, go through door alone). Without new_location_id the object does not move. Omit if the object stays in place. When you set new_location_id to "player": narrative_prose uses exactly "The player now holds the [object]."; that object's prose_impact exactly "Taken by player." Do not narrate or encode a transfer you did not warrant.
...
GIVE TO NPC: Set the object's new_location_id to the **NPC** only when giving is **warranted** (explicit give or dialogue/action you resolve as a real handover). If they only mention or offer without giving, the NPC does not receive the item.
  ...
  "node_impacts": [
    {
      ...
      "new_location_id": "<optional: set an object's new_location_id to \"player\" when transfer is **warranted** (explicit take, NPC gives, dialogue commits to receiving/picking up, etc.)—never on look or **pure movement** alone (east, go through door with no object interaction). For MOVEMENT set the player entry's new_location_id to the exit target. Only use 'The player now holds…' / 'Taken by player.' when you set that object's new_location_id to \"player\">"
    }
  ],
