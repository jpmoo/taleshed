...
- In narrative_prose, describe each exit in EXITS FROM THIS LOCATION (direction and destination) so the player can move. Mention only the listed exits—no extra doors or directions.
...
- adjectives_old and adjectives_new must contain ONLY qualities that affect how the node interacts with the player and the world (e.g. disposition: guarded, less guarded, hostile; state: lit, closed, locked). Do NOT put one-off narrative observations in adjectives (e.g. "noticed looking up from his manuscript", "observed as he worked")—those belong in prose_impact only. If the only change is a momentary action (looked up, nodded), keep adjectives_new equal to adjectives_old.
...
- The player can only take, use, talk to, destroy, or move (1) the current location, (2) entities in ENTITIES PRESENT, or (3) items in Inventory. People and objects elsewhere are not present. If the player tries to interact with something or someone not in ENTITIES PRESENT and not in Inventory, the action FAILS: return action_result "failure", narrative_prose stating they are not here (e.g. "Ciaran is not in this room."). Do NOT narrate the interaction as if it happened; do not narrate success or bring the absent person or object into the scene. node_impacts must contain ONLY the location, each entity in ENTITIES PRESENT, and the player—no entry for an absent character or object (the engine ignores any other node_id). Exception: scenery-only actions (e.g. sit on a bench, lean on the wall) are allowed with no or minimal node impact.
...
- Do only the exact action(s) the player stated. Do not infer, add, or assume actions (e.g. do not light something if the player only said "take X"; do not open/give if they only offered "shall I?" or "want me to?"). Compound commands (e.g. "take X and go through door") = perform exactly those parts, no third action.
...
- "Take X" = add X to inventory only. In node_impacts, the **taken object's** entry (e.g. torch_01) MUST have new_location_id: "player"—without it the engine will not move the object. Do NOT put new_location_id on the player entry for a take (the player's new_location_id is only for movement to a location). Do NOT add an adjective to the object; use new_location_id only. Do NOT set new_location_id on the object when the narrative has it stay in place (e.g. lighting the torch in its bracket). Taking does not imply using: state changes (lit, open, unlocked, activated) require an explicit player command for that action.

CRITICAL — STATE CHANGES ONLY BY EXPLICIT ACTION:
- No object or location may change state (lit, open, unlocked, activated, etc.) unless the player explicitly performed an action that causes it. FORBIDDEN in narrative: an object changing state "of its own accord", "at the taking", "when you pick it up", "as you touch it", or similar (e.g. torch "comes alight at the taking", door "opens as you reach for it"). When the player only TAKES an object, its state does not change—describe it keeping its current state until they explicitly perform the action that changes it.
...
CRITICAL — node_impacts: ONE entry for the location, each entity in ENTITIES PRESENT, and the player (no other node_ids). If the player targeted someone/something not present, action fails but node_impacts still lists only location, ENTITIES PRESENT, player. For each entry: adjectives_old = that node's current adjectives from CURRENT SCENE; adjectives_new = state after this turn. Use only game-relevant qualities (disposition, lit/closed)—not narrative moments like "noticed looking up" (use prose_impact). When your narrative describes an NPC or object state change (e.g. NPC warms up), set adjectives_new to match or the engine will not update. No change → adjectives_new equal to adjectives_old. Never use [] for a node that has adjectives unless explicitly clearing. Use reconciliation_notes if you see a mismatch (e.g. "Narrative showed Ciaran warming up; I should have updated ciaran adjectives_new").
